...
def all_vowel_pairs(words)
    ...
    words.each_with_index do |word1, idx1|
        words.each_with_index do |word2, idx2|
            pairs << word1 + ' ' + word2 if idx2 > idx1
        end
    end
    pairs.each do |pair|
        true_pairs << pair if all_vowel?(pair)
    end
    true_pairs
end
...
def all_vowel?(pair)
    vowel = 'aeiou'
    vowel.each_char do |char|
        return false if !pair.include?(char)
    end
    true
end
...
class String
    ...
    def substrings(length = nil)
        ...
        arr.select { |sub| sub.length == length} 
    end
